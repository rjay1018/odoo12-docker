...
class OmnaSyncCategories(models.TransientModel):
    ...
    def sync_categories(self):
        try:
            ...
            for category in categories:
                ...
                self.category_tree(arr,False,category_id,self.integration_id.id,category_obj)
    ...
    def category_tree(self, arr, parent_id, category_id, integration_id, category_obj):
        if len(arr) == 1:
            ...
            if not c:
                category_obj.create({'name': name, 'omna_category_id': category_id,
                                     'parent_id': parent_id, 'integration_id': integration_id})
