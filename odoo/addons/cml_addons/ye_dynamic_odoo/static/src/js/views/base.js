...
"use strict";
    ...
    var EditBase = WidgetBase.extend({
        ...
        prepareNewField: function () {
            const newField = this.ref.content.newField || {};
            return Object.values(newField).map((field) => {
                ...
                if (["many2many", "one2many", "many2one"].includes(type)) {
                    ...
                    if (type == "one2many" && !relation_field) {
                        alert("Pls Choose Relation field. Thanks !");
                        return false;
                    }
    ...
    var PropertyBase = WidgetBase.extend({
        ...
        start: function () {
            ...
            this.components = {component: {label: "Tags", class: "_wComTag", type: "component"}, fields: {label: "Fields", class: "_wComField", type: "fieldNew"}};
            ...
            property.relation = {label: "Relation", widget: FieldBasic.Input};
            property.context = {label: "Context", widget: FieldBasic.Input};
            property.relation_field = {label: "Relation Field", widget: FieldBasic.Input};
            ...
            view.form.field = {};
            view.form.field.default = ["required", "invisible", "readonly", "nolabel", "widget", "string", "help", "placeholder", "context"];
            view.form.field.char = ["required", "invisible", "readonly", "nolabel", "widget", "string", "help", "placeholder", "context"];
            view.form.field.selection = ["required", "invisible", "readonly", "nolabel", "widget", "string", "selection", "help", "context"];
            view.form.field.boolean = ["required", "invisible", "readonly", "nolabel", "widget", "string", "help", "context"];
            view.form.field.binary = ["required", "invisible", "readonly", "nolabel", "widget", "string", "help", "context"];
            view.form.field.many2one = ["required", "invisible", "readonly", "can_create", "can_write", "nolabel", "relation", "widget",  "string", "help", "domain", "context"];
            view.form.field.one2many = ["required", "invisible", "readonly", "nolabel", "relation", "relation_field", "widget", "string", "help", "context"];
            view.form.field.many2many = ["required", "invisible", "readonly", "nolabel", "relation", "widget", "string", "help", "domain", "context"];
            view.form.tree = ["showInvisible", "editable", "create", "delete", "string", "color"];
            view.form.label = ["invisible", "string"];
            view.form.page = ["invisible", "string"];
            view.form.group = ["invisible", "string"];
            view.form.form = ["showInvisible", "string"];
            view.form.button = ["invisible", "string"];
            view.tree.field = ["required", "invisible", "readonly", "string", "widget"];
            view.tree.tree = ["showInvisible", "editable", "search_advance", "create", "delete", "sticky", "string", "color"];
        ...
        onClickTab: function () {},
        renderTab: function () {
            this.ref.tab = new FieldBasic.Tab(this, {tabs: this.tabs, data: this.state.tab, onClickTab: this.onClickTab.bind(this)});
            this.ref.tab.renderElement();
            this.$el.find("._wEPTab").empty().append(this.ref.tab.$el);
        },
        _renderTabWField: function () {
            const {bindSortable, viewInfo} = this.props, {fieldsGet} = viewInfo,
        ...
        _renderTabProperty: function () {
            const {controller, viewInfo} = this.props;
        ...
        _renderTabComponent: function () {
            ...
            wConComponent.find('._wComCon').append(Object.keys(this.components).map((comName) =>
                QWeb.render("ViewEdit.TabComponent.Com", this.components[comName])
            ));
