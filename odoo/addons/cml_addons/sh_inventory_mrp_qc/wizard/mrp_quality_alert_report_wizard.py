...
class MrpQualityAlertReport(models.AbstractModel):
    _name = 'report.sh_inventory_mrp_qc.mrp_quality_alert_doc'
    _description = "MRP quality alert report abstract model"

    @api.model
    def _get_report_values(self, docids, data=None):
        quality_alert_obj = self.env['sh.mrp.quality.alert']
                        ...
                        for alert in search_alerts:
                            ...
                            if alert.workorder_id:
                                alert_dic = {
                                    ...
                                    'workorder_id': alert.workorder_id.name,
...
class MrpQualityAlertReportWizard(models.TransientModel):
    _name = 'mrp.quality.alert.report'
    ...
    operation_type = fields.Selection(
        [('manufacturing', 'manufacturing'), ('workorder', 'workorder')], 'Is Operation')

    def print_report(self):
        datas = self.read()[0]
        return self.env.ref('sh_inventory_mrp_qc.mrp_report_quality_alert_action').report_action([], data=datas)
