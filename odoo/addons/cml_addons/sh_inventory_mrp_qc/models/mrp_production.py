...
class Manufacturing(models.Model):
    ...
    sh_mrp_quality_alert_ids = fields.One2many(
        'sh.mrp.quality.alert', 'mrp_id', string="Mrp Quality Checks")

    sh_mrp_quality_check_ids_nbr = fields.Integer(string='# of Sales Orders')
        # compute='_compute_sh_mrp_quality_check_ids_nbr', )
    ...
    def _get_qc_hide(self):
        for rec in self:
            ...
            for i in qc:
                ...
                if i.control_point_id.number_of_test == 0 and \
                        i.control_point_id.id not in [l.control_point_id.id for l in qc.filtered(lambda i:i.state == 'pass')]:
                    rec.qc_button_hide = False
                    count = 0
                    break

                elif i.control_point_id.number_of_test == 0 and \
                        i.control_point_id.id in [l.control_point_id.id for l in qc.filtered(lambda i:i.state == 'pass')]:
                    rec.qc_button_hide = True
                elif i.control_point_id.number_of_test == self.qc_total_count and i.control_point_id.id \
                        in list1:
                    count += 1
        ...
        if self:
            for rec in self:
                ...
                last_quality_check = self.env['sh.mrp.quality.check'].sudo().search(
                    [('product_id', '=', rec.product_id.id), ('sh_mrp', '=', rec.id)],
                ...
                if last_quality_check:
                    for qc in last_quality_check:
                        ...
                        if qc.state == 'fail' and qc.control_point_id.id not in \
                                [i.control_point_id.id for i in rec.sh_mrp_quality_check_ids.filtered(lambda i: i.state =='pass')]:
                            rec.qc_fail = True
                            rec.qc_pass = False
                            rec.need_qc = False
                            rec.pending_qc = False
    ...
    def quality_point(self):
        if self.product_id:
            ...
            quality_point_id = self.env['sh.qc.point'].sudo().search([
                ...
                ('operation.id', '=', self.picking_type_id.id),
            ...
            if len(self.sh_mrp_quality_check_ids) >= self.sh_mrp_quality_point_id.number_of_test and self.sh_mrp_quality_point_id.number_of_test != 0 \
                    and not self.sh_mrp_quality_check_ids.filtered(lambda i: i.state == 'fail'):

                message = 'You Maximum number of allowed Test is Over Now'

            elif self.sh_mrp_quality_check_ids.filtered(lambda i: i.state == 'fail'):
                self.count += 1
                for i in self.sh_mrp_quality_check_ids.filtered(lambda i: i.state == 'fail'):

                    if i.control_point_id.id in [i.control_point_id.id for i in self.sh_mrp_quality_check_ids.filtered(lambda i: i.state == 'pass')]:
                        message = "dsfmsdkfm"
                        ...
                        return {
                            ...
                            'context': {'default_mrp_id': i.sh_mrp.id, 'default_point_id': i.control_point_id.id,
                                        'default_transfer': quality_point_id.operation.id,
                ...
                return {
                    ...
                    'context': {'default_mrp_id': self.id, 'default_transfer': quality_point_id.operation.id, 'default_point_id': quality_point_id.id},
                    'target': 'new',
                }
