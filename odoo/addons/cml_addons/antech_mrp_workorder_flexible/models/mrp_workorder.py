...
class MrpWorkorder(models.Model):
    ...
    _inherit = ['mrp.workorder', 'mail.thread', 'mail.activity.mixin', 'mrp.abstract.workorder']
    ...
    active_move_line_ids = fields.One2many('stock.move.line', 'workorder_id', domain=[('done_wo', '=', False)])
            ...
            for workorder in self:
                ...
                if workorder == workorder.production_id.workorder_ids[0] and 'date_planned_start' in values:
                    workorder.production_id.with_context(force_date=True).write({
                        'date_planned_start': fields.Datetime.to_datetime(values['date_planned_start']) or fields.Datetime.now()
                    })
                if workorder == workorder.production_id.workorder_ids[-1] and 'date_planned_finished' in values:
                    workorder.production_id.with_context(force_date=True).write({
                        'date_planned_finished': fields.Datetime.to_datetime(values['date_planned_finished']) or fields.Datetime.now()
                    })
    ...
    def record_production(self):
        ...
        self.ensure_one()
        # is_more_consumed = False
        # for line in self.raw_workorder_line_ids:
        #     if line.qty_done > line.qty_reserved:
        #         raise UserError("Can not process workorder because there is consumed qty more than reserved")
        ...
        material_not_enough = False
        error_msg = ''
        for move in self.move_raw_ids:
            if move.state not in ['assigned', 'done']:
                error_msg += '\n - {product} needs {demand} {uom} but only {reserved} {uom} available.'.format(
                    product=move.product_id.name,
                    demand=move.product_uom_qty,
                    uom=move.product_uom.name,
                    reserved=move.reserved_availability
                )
                material_not_enough = True
        if material_not_enough:
            raise UserError("Can not finish this workorder because there is not enough material: %s" % error_msg)
    ...
    def _create_or_update_finished_line(self):
        ...
        for workorder in (self.production_id.workorder_ids - self):
            ...
            quantity = sum(line.mapped('qty_done')) + quantity_remaining
