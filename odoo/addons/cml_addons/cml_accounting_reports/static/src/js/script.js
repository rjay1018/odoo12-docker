odoo.define('cml_accounting_reports.DynamicTbMain', function (require) {
'use strict';
...
self.$el.find('.salesperson-multiple').select2({
...
k.date_invoice = field_utils.format.date(field_utils.parse.date(k.date_invoice, {}, {isUTC: true}));
});
$(event.currentTarget).parent().parent().parent().find('.py-mline-table-div').remove();
$(event.currentTarget).parent().parent().find('a').css({'background-color': 'white','font-weight': 'normal'});
$(event.currentTarget).parent().parent().after(
...
var sales_person_ids = [];
var salesperson_list = $(".salesperson-multiple").select2('data')
for (var i = 0; i < salesperson_list.length; i++) {
    sales_person_ids.push(parseInt(salesperson_list[i].id))
}
output.salesperson_ids = sales_person_ids
...
if(options_list[i].id === 'bal_all'){
output.display_accounts = 'all';}
else if(options_list[i].id === 'bal_not_zero'){
output.display_accounts = 'balance_not_zero';}
else if(options_list[i].id === 'bal_zero'){
output.display_accounts = 'bal_zero';}
//else if((options_list[i].id === 'bal_not_zero') || (options_list[i].id === 'bal_zero')){
//output.display_accounts = 'all';}
