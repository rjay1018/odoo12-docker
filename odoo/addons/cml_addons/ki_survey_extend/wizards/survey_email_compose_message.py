...
class SurveyMailComposeMessage_inherit(models.TransientModel):
    _inherit = 'survey.mail.compose.message'

    public = fields.Selection(selection_add = [('email_relative','Send To Guardian or Relatives')])
    relative_ids = fields.Many2many(
        'res.partner.relative'
    )
    relative_partner_id = fields.Many2one(
        'res.partner',
        'Existing Contact',
        placeholder="Add Existing Contacts"
    )
    exclude_partner = fields.Boolean(
        "Exclude Partner",
        default=False
    )

    @api.onchange('relative_partner_id')
    def set_partner_id(self):
        for rec in self:
            if rec.relative_partner_id:
                rec.partner_ids = [(6,0,rec.relative_partner_id.ids)]

    @api.onchange('partner_ids','relative_ids')
    def set_relative_list(self):
        if self.partner_ids:
            ls = []
            for rec in self.partner_ids:
                ls.extend(rec.relatives_ids.ids)
            self.relative_ids = [(6,0,list(set(ls)))]
        if self.relative_ids and self.public == 'email_relative':
            self.multi_email = ",".join([str(line.email) for line in self.relative_ids])
            if self.multi_email:
                self.onchange_multi_email()

    @api.multi
    def send_mail_action(self):
        if self.public != 'email_relative':
            return super(SurveyMailComposeMessage_inherit, self).send_mail_action()
        else:
            return self.send_mail_new()

    @api.multi
    def send_mail_new(self, auto_commit=False):
        ...
        def create_token(wizard, partner_id, email):
            ...
            if context.get("survey_resent_token"):
                ...
                if survey_user_input:
                    return survey_user_input.token
            if wizard.public not in ['email_private','email_relative']:
                return None
            else:
                token = pycompat.text_type(uuid.uuid4())
                # create response with token
                relation_typ = 'self'
                if email and self.relative_ids:
                    relat_id = self.relative_ids.filtered(lambda l:str(l.email) == str(email))
                    if relat_id:
                        relation_typ = relat_id.relation_type
                survey_user_input = SurveyUserInput.create({
                    ...
                    'state': 'new',
                    'token': token,
                    'relation_type' : relation_typ,
                    'partner_id': relat_id.partner_id.id,
                    'email': email})
                return survey_user_input.token
