...
class MrpBom(models.Model):
    ...
    def create(self, vals):
        ...
        for material in res.bom_line_ids:
            ...
            for i in product_id:
                cost = i.uom_id._compute_price(material.product_id.standard_price, material.product_uom_id)
            vals = {
    ...
    def write(self, vals):
        ...
        for material in self.bom_line_ids:
            ...
            if self.bom_material_cost_ids.filtered(lambda i: i.product_id == material.product_id.product_tmpl_id):
                pass
            else:
                product_id = self.env['product.template'].browse(material.product_id.product_tmpl_id.id)
                cost = product_id.uom_id._compute_price(material.product_id.standard_price, material.product_uom_id)
...
class MrpBomMaterialCost(models.Model):
    ...
    # @api.model
    # def write(self, vals):
    #     res = super(MrpBomMaterialCost, self).write(vals)
    #     return res
...
class MrpProduction(models.Model):
    ...
    def create(self, vals):
        ...
        for rec in res.bom_id.bom_material_cost_ids:
            cost = rec.product_id.uom_id._compute_price(rec.product_id.standard_price, rec.uom_id)
            vals = {
                'product_id': rec.product_id.id,
                'planned_qty': rec.planned_qty,
                'uom_id': rec.uom_id.id,
                'cost': cost,
                'operation_id': rec.operation_id.id or False,
        ...
        for wizard in res:
            ...
            for material in production.pro_material_cost_ids:
                for bom in production.bom_id.bom_material_cost_ids:
                    if material.product_id.id == bom.product_id.id:
                        material.write(
                            {'planned_qty': wizard.product_qty * bom.planned_qty / bom.mrp_bom_material_id.product_qty})

            for labour in production.pro_labour_cost_ids:
                for bom in production.bom_id.bom_labour_cost_ids:
                    if labour.operation_id.id == bom.operation_id.id:
                        labour.write(
                            {'planned_qty': wizard.product_qty * bom.planned_qty / bom.mrp_bom_labour_id.product_qty})

            for overhead in production.pro_overhead_cost_ids:
                for bom in production.bom_id.bom_overhead_cost_ids:
                    if overhead.operation_id.id == bom.operation_id.id:
                        overhead.write(
                            {'planned_qty': wizard.product_qty * bom.planned_qty / bom.mrp_bom_overhead_id.product_qty})

        return res

    @api.depends('pro_material_cost_ids.total_cost', 'pro_material_cost_ids.total_actual_cost',
                 'pro_labour_cost_ids.total_cost',
                 'pro_labour_cost_ids.total_actual_cost', 'pro_overhead_cost_ids.total_cost',
                 'pro_overhead_cost_ids.total_actual_cost')
    def _compute_total_cost(self):
        material_total = 0.0
    ...
    @api.depends('total_material_cost', 'total_labour_cost', 'total_overhead_cost', 'total_actual_material_cost',
                 'total_actual_labour_cost', 'total_actual_overhead_cost')
    def _compute_total_all_cost(self):
        total = 0.0
        actual_total = 0.0
        for rec in self:
            total = rec.total_material_cost + rec.total_labour_cost + rec.total_overhead_cost
            actual_total = rec.total_actual_material_cost + rec.total_actual_labour_cost + rec.total_actual_overhead_cost
            rec.total_all_cost = total
            print ("45============================", actual_total)
            rec.total_actual_all_cost = actual_total
        ...
        for rec in self:
            for line in rec.finished_move_line_ids:
                if line.qty_done != 0.0:
                    total = rec.total_actual_all_cost / line.qty_done
            rec.product_unit_cost = total
    ...
    # Costing Tab
    total_material_cost = fields.Float(compute='_compute_total_cost', string="Total Planned Material Cost", default=0.0)#string="Total Material Cost"
    total_labour_cost = fields.Float(compute='_compute_total_cost', string="Total Planned Labour Cost", default=0.0)#string="Total Labour Cost"
    total_overhead_cost = fields.Float(compute='_compute_total_cost', string="Total Planned Overhead Cost", default=0.0)#string="Total Overhead Cost"
...
class MrpWorkorder(models.Model):
    ...
    def _compute_total_cost(self):
        ...
        for order in self:
            ...
            for line in order.wo_material_cost_ids:
                print("--------------------------wo_material_cost_ids line",line)
                material_total += line.total_cost
                material_actual_total += line.total_actual_cost
            ...
            for line in order.wo_labour_cost_ids:
                print("--------------------------wo_labour_cost_ids line", line)
                labour_total += line.total_cost
                labour_actual_total += line.total_actual_cost
            ...
            for line in order.wo_overhead_cost_ids:
                print("--------------------------wo_overhead_cost_ids line", line)
                overhead_total += line.total_cost
                overhead_actual_total += line.total_actual_cost
    ...
    def record_production(self):
        print("------------------------record_prodcution_calling")
        print("------------------------self.production_id",self.production_id)
        for line in self.production_id.pro_overhead_cost_ids:
            print("---------------line",line)
            print("---------------self.duration",self.duration)

            line.write({'actual_qty': self.duration})

        print("------------------------self.production_id.pro_labour_cost_ids",self.production_id.pro_labour_cost_ids)
        ...
        print("------------------------self.production_id.pro_overhead_cost_ids", self.production_id.pro_overhead_cost_ids)
        for line in self.production_id.pro_overhead_cost_ids:
            print("---------------line", line)
            if line.operation_id.id == self.operation_id.id:
                line.write({'actual_qty': self.actual_hour_wo})
        ...
        # for line in self.production_id.pro_labour_cost_ids:
        #     if line.operation_id.id == self.operation_id.id:
        #         line.actual_qty = self.time_ids.duration
        print("res-------------------------",res)
        return res
