class amChartsComponent {
    constructor(attr){
        ...
        // Scorecard Basic customization
        this.scorecardBgColor = attr.scorecardBgColor;
        this.scorecardIconType = attr.scorecardIconType;
    ...
    makeScorecardBasic() {
        var self = this;
        var $container = $("#" + self.idElm);
        var currencyCode = self.currencyCode || "";
        var particle = self.particle || "";
        
        // Get customization configs
        var bgColor = self.scorecardBgColor || 'default';
        var iconType = self.scorecardIconType || 'none';
        var iconColor = self.scorecardIconColor || 'primary';

        // Clean previous background classes so changes apply immediately
        $container.removeClass(function (idx, cls) {
            return (cls && cls.match(/(^|\s)scorecard-bg-\S+/g) || []).join(' ');
        });

        var recentValue = (self.data).length ? (self.data[(self.data).length - 1])[self.metric[0]] : 0;
        
        // Build icon HTML if icon type is selected (Font Awesome 4.7.0)
        var iconHTML = '';
        if (iconType !== 'none') {
            // Support both 'fa-iconname' and 'iconname' formats
            var faClass = iconType.startsWith('fa-') ? iconType : 'fa-' + iconType;
            iconHTML = `<i class="fa ${faClass} scorecard-icon scorecard-icon-${iconColor}"></i>`;
        }

        $container.addClass(`scorecard scorecard-sm scorecard-bg-${bgColor}`);
        $container.append(
        `
            ...
            <div class="scorecard-img-container">
                ${iconHTML}
            </div>
            `
            );
