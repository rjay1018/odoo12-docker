    ...
    var IZIConfigDashboard = Widget.extend({
        ...
        _onClickSelectTheme: function (ev) {
            ...
            if (theme_id && theme_name) {
                Dialog.confirm(this, _t('Do you confirm to change the dashboard theme?'), {
                    confirm_callback: function () {
                        ...
                        self._rpc({
                            ...
                            Dialog.alert(self, _t('Dashboard theme has been changed successfully'));
                ...
                if (layout) {
                    self._rpc({
                        ...
                        if (result.status == 200) {
                            ...
                            self._loadFilteredDashboard();
                            Dialog.alert(self, _t('Dashboard layout has been saved successfully.'));
                        }
                    });
                }
            }
        ...
        _onClickSaveDashboard: function(ev) {
            ...
            if (self.selectedDashboard && name) {
                Dialog.confirm(this, _t('Do you confirm to change the dashboard information?'), {
                    confirm_callback: function () {
                        var data = {
                            'name': name,
                        };
                        self._rpc({
                            ...
                            Dialog.alert(self, _t('Dashboard has been saved successfully'));
                            self.$titleDashboard.text(name);
        ...
        _onClickDeleteDashboard: function(ev) {
            ...
            ev.stopPropagation();
            if (self.selectedDashboard) {
                Dialog.confirm(this, _t('Do you confirm to delete the dashboard ?'), {
                    confirm_callback: function () {
                        self._rpc({
                            model: 'izi.dashboard',
                            method: 'unlink',
                            ...
                            Dialog.alert(self, _t('Dashboard has been deleted successfully'));
                            self._initDashboard();
                        });
                    },
                });
        ...
        _onClickExportCapture: function (ev) {
            ...
            if (self.selectedDashboard) {
                ...
                html2canvas(document.querySelector('.izi_view_dashboard'), {useCORS: true, allowTaint: false}).then(function(canvas){
                    ...
                    Dialog.alert(self, _t('Dashboard has been captured.'));
                    btn.button('reset');
