...
class IZIDashboardFilter(models.Model):
    ...
    source_type = fields.Selection([('model', 'Model'), ('predefined', 'Predefined')], string="Source Type", required=True)
...
class IZIDashboardFilterAnalysis(models.Model):
    ...
    allowed_analysis_ids = fields.Many2many('izi.analysis', related='filter_id.dashboard_id.analysis_ids')
    analysis_id = fields.Many2one('izi.analysis', string="Analysis", required=True, ondelete='cascade')
    allowed_field_ids = fields.One2many('izi.table.field', string="Analysis Fields", related='analysis_id.table_id.field_ids')
    ...
    def create(self, vals):
        ...
        if rec.dashboard_id:
            action = self.env['ir.actions.act_window'].create({
                'name': 'Action %s %s' % (rec.dashboard_id.display_name, rec.dashboard_id.id),
