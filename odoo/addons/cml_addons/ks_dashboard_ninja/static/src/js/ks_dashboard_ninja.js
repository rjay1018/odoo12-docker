odoo.define('ks_dashboard_ninja.ks_dashboard', function (require) {
    ...
    var KsDashboardNinja = AbstractAction.extend(ControlPanelMixin, {
        ...
        onKsDashboardMenuContainerShow: function (e) {
          ...
          if (this.chart_container[chart_id]){
            $(e.currentTarget).addClass('ks_dashboard_item_menu_show');
            var item_id = $(e.currentTarget).find('.ks_chart_inner_buttons.ks_dashboard_more_action').data('itemId');
        ...
        ksUpdateDashboardItem : function(ids) {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (item_data['ks_dashboard_item_type'] == 'ks_list_view') {
                    ...
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboarditem_id.ks_dashboard_item.ks_dashboard_item_hover").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else if (item_data['ks_dashboard_item_type'] == 'ks_kpi') {
                    var item_view = self.renderKpi(item_data);
                    self.$el.find(".grid-stack-item[data-gs-id=" + item_data.id + "]").find(".ks_dashboard_kpi.ks_dashboard_kpi_dashboard.ks_dashboard_custom_srollbar.ks_dashboarditem_id.ks_dashboard_item_hover").replaceWith($(item_view).find('.ks_dashboarditem_id'));
                } else {
